...
export default class SettingsController {
  public async getSettings(token: any): Promise<Object | String> {
    if (!token) return "Invalide Token";
    try {
      const data = jwt.verify(token, process.env.TOKEN_KEY || "123456789");
  ...
  public async setSettings(token: any, @Body() body: any): Promise<String> {
    if (!token) return "Invalide Token";
    try {
