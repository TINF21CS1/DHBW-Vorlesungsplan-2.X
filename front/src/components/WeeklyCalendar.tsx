...
import { Grid, Stack, Box, Typography, IconButton, TextField, Autocomplete } from "@mui/material";
...
const WeeklyCalendar = () => {
    ...
    if (isLoading && selectedCourse) {
        fetch("/api/calender/" + selectedCourse + "/" + currentWeek.toISOString() + "/" + weekEnd.toISOString() + "/").then(response => {
    ...
    return <Box onTouchStart={touchstart} onTouchEnd={touchend}>
        <Stack justifyContent="center" direction="row">
            <div style={{
                ...
                <IconButton size="large" onClick={() => { setCurrentWeek(startOfWeek(add(currentWeek, { days: -7 }), { weekStartsOn: 1 })); setIsLoading(true) }} color="secondary" aria-label="letzte Woche">
                    <ChevronLeft />
                </IconButton>
                <Typography align="center" >KW {format(currentWeek, "ww")}</Typography>
                <IconButton size="large" onClick={() => { setCurrentWeek(startOfWeek(add(currentWeek, { days: 7 }), { weekStartsOn: 1 })); setIsLoading(true) }} color="secondary" aria-label="nächste Woche">
                    <ChevronRight />
                </IconButton>
            </div>
        </Stack>
        ...
        {isLoading && selectedCourse && <Typography align="center">Loading...</Typography>}
        {!selectedCourse && 
            <Autocomplete
            disablePortal
            id="kurs-combobox"
            ...
            sx={{ width: 300 }}
            renderInput={(params) => <TextField {...params} label="Kurs auswählen" />}
